...
#define WIGWAG_CHECK_THROW_ANYTHING(EXPR) \
    do { \
        bool _threw = false; \
        try { EXPR; } catch (...) { _threw = true; } \
        BOOST_CHECK_MESSAGE(_threw, "Expected an exception to be thrown"); \
    } while (false)
...
namespace wigwag_test_detail
{
    ...
    void do__test__life_assurance__common()
    {
        {
...

        {
            ...
            auto disconnect_time = duration_cast<milliseconds>(p.reset()).count();
            BOOST_CHECK(!handler_invoked.get());
            BOOST_CHECK_LE(disconnect_time, 100);
        }
...
BOOST_AUTO_TEST_SUITE(wigwag_api_v1_test)
...
BOOST_AUTO_TEST_CASE(test_signals)
{
    ...
    s(1);
    { auto l = lock(m); BOOST_CHECK_EQUAL(value, 1); }

    token t1 = s.connect([&](int i) { auto l = lock(m); value += 10 * i; });
    s(3);
    { auto l = lock(m); BOOST_CHECK_EQUAL(value, 34); }

    t0.reset();
    s(5);
    { auto l = lock(m); BOOST_CHECK_EQUAL(value, 84); }

    token t2 = s.connect(worker, [&](int i) { auto l = lock(m); value -= i; });
    s(7);
    thread::sleep(100);
    { auto l = lock(m); BOOST_CHECK_EQUAL(value, 147); }
}

BOOST_AUTO_TEST_CASE(test_signal_connector)
{
    signal<void(int)> s;
...
BOOST_AUTO_TEST_CASE(test_listenable)
{
    using wigwag_test_detail::test_listener;
...
BOOST_AUTO_TEST_CASE(test_token)
{
    ...
    s();
    BOOST_CHECK_EQUAL(counter, 101);
}

BOOST_AUTO_TEST_CASE(test_token_pool)
{
    signal<void()> s;
    token_pool tp;
    int counter = 0;
...
BOOST_AUTO_TEST_CASE(test_connect_from_handler)
{
    using wigwag_test_detail::test_listener;
    {
        token_pool tp;
        ...
        s();
        BOOST_CHECK(!second_handler_invoked);
        s();
        BOOST_CHECK(second_handler_invoked);
    }
    {
...
BOOST_AUTO_TEST_CASE(test_disconnect_from_handler)
{
    using wigwag_test_detail::test_listener;
    {
        signal<void()> s;
...
BOOST_AUTO_TEST_CASE(test_signal_attributes)
{
    std::shared_ptr<task_executor> worker = std::make_shared<thread_task_executor>();
    token_pool tp;
    ...
    {
        signal<void()> s;
        BOOST_CHECK_NO_THROW(tp += s.connect([]{}));
        BOOST_CHECK_NO_THROW(tp += s.connect(worker, []{}));
    }
...

    {
        ...
        BOOST_CHECK_NO_THROW(tp += s.connect([]{}));
        BOOST_CHECK_THROW((tp += s.connect(worker, []{})), std::runtime_error);
    }

    {
        signal<void()> s(signal_attributes::connect_async_only);
        BOOST_CHECK_THROW((tp += s.connect([]{})), std::runtime_error);
        BOOST_CHECK_NO_THROW(tp += s.connect(worker, []{}));
    }
}
...
BOOST_AUTO_TEST_CASE(test_handler_attributes)
{
...

    {
        ...
        BOOST_CHECK_EQUAL(sync_state, 1);
        s(2);
        BOOST_CHECK_EQUAL(sync_state, 2);
        t.reset();
        BOOST_CHECK_EQUAL(sync_state, 3);
    }

    {
        ...
        thread::sleep(100);
        BOOST_CHECK_EQUAL(async_state.get(), 1);
        s(2);
        thread::sleep(100);
        BOOST_CHECK_EQUAL(async_state.get(), 2);
    }

    {
        int sync_state = 0;
        token t = s.connect([&](int i) { sync_state = i; }, handler_attributes::suppress_populator);
        BOOST_CHECK_EQUAL(sync_state, 0);
        s(2);
        BOOST_CHECK_EQUAL(sync_state, 2);
        t.reset();
        BOOST_CHECK_EQUAL(sync_state, 2);
...
BOOST_AUTO_TEST_CASE(test__exception_handling__default)
{
    using wigwag_test_detail::test_listener;
    signal<void()> s;

    {
        ...
        BOOST_CHECK_THROW(s(), std::runtime_error);
    }

    {
        token t = s.connect([&] { throw 0; });
        WIGWAG_CHECK_THROW_ANYTHING(s());
    }
...

    {
        ...
        BOOST_CHECK_THROW(l.invoke([](const test_listener& l){ l.f(); }), std::runtime_error);
    }

    {
        token t = l.connect(test_listener([]{}, [&] (int i) { throw i; }));
        WIGWAG_CHECK_THROW_ANYTHING(l.invoke([](test_listener& l){ l.g(42); }));
    }
}

BOOST_AUTO_TEST_CASE(test__exception_handling__none)
{
    using wigwag_test_detail::test_listener;
...
BOOST_AUTO_TEST_CASE(test__exception_handling__print_to_stderr)
{
    using wigwag_test_detail::test_listener;
    {
...

    {
        ...
        BOOST_CHECK_NO_THROW(l.invoke([](const test_listener& l){ l.f(); }));
        WIGWAG_CHECK_THROW_ANYTHING(l.invoke([](test_listener& l){ l.g(42); }));
    }
}
...
BOOST_AUTO_TEST_CASE(test__state_populating__default)
{
    using h_type = const std::function<void(int)>&;
    ...
    BOOST_CHECK_EQUAL(state.get(), 1);
    thread::sleep(1000);
    BOOST_CHECK_EQUAL(state.get(), 2);
    t.reset();
    BOOST_CHECK_EQUAL(state.get(), 2);
}

BOOST_AUTO_TEST_CASE(test__state_populating__populator_only)
{
    using h_type = const std::function<void(int)>&;
...
BOOST_AUTO_TEST_CASE(test__state_populating__populator_and_withdrawer)
{
    using h_type = const std::function<void(int)>&;
    ...
    BOOST_CHECK_EQUAL(state.get(), 2);
    t.reset();
    BOOST_CHECK_EQUAL(state.get(), 3);
}

BOOST_AUTO_TEST_CASE(test__state_populating__none)
{
    int signal_state = 0;
...
BOOST_AUTO_TEST_CASE(test__life_assurance__default)
{ wigwag_test_detail::do__test__life_assurance__common<signal<void()>>(); }

BOOST_AUTO_TEST_CASE(test__life_assurance__intrusive_life_tokens)
{ wigwag_test_detail::do__test__life_assurance__common<signal<void(), exception_handling::default_, threading::default_, state_populating::default_, life_assurance::intrusive_life_tokens>>(); }

BOOST_AUTO_TEST_CASE(test__life_assurance__single_threaded)
{
    {
...
BOOST_AUTO_TEST_CASE(test__life_assurance__none)
{
...
BOOST_AUTO_TEST_CASE(test__creation__ahead_of_time)
{
    using wigwag_test_detail::test_listener;
    {
...
BOOST_AUTO_TEST_CASE(test__creation__lazy)
{
    using wigwag_test_detail::test_listener;
    {
...
BOOST_AUTO_TEST_CASE(test_life_token)
{
    {
        life_token lt;
...

    {
        ...
        auto release_time = duration_cast<milliseconds>(p.reset()).count();
        BOOST_CHECK_LE(move_time, 100);
        BOOST_CHECK_LE(release_time, 100);
    }
...
BOOST_AUTO_TEST_CASE(test_task_executors)
{
    {
        std::shared_ptr<task_executor> worker = std::make_shared<thread_task_executor>();
        ...
        std::lock_guard<std::mutex> l(m);
        BOOST_CHECK_NE(handler_thread_id, std::thread::id());
        BOOST_CHECK_NE(handler_thread_id, std::this_thread::get_id());
    }
...
BOOST_AUTO_TEST_CASE(test_task_executor_function_copying)
{
    using wigwag_test_detail::copy_ctor_counter;
...
BOOST_AUTO_TEST_CASE(test_signal_handler_copying)
{
    using wigwag_test_detail::copy_ctor_counter;
#if !HAS_STD_FUNCTION_MOVE_BUG
    {
...
BOOST_AUTO_TEST_CASE(test_signal_parameters_copying)
{
    using wigwag_test_detail::copy_ctor_counter;
#if !HAS_STD_FUNCTION_MOVE_BUG
    {
